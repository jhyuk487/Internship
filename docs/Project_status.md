...
- **현재 상태**: 핵심 기능 구현 완료, 서비스 안정화 및 게스트 접근성 개선 완료

## 2. 주요 업데이트 사항 (2026-02-10)
- **보안 강화** (2026-02-09): 비밀번호 `bcrypt` 해싱 도입 및 기존 데이터 500건 일괄 마이그레이션 완료
- **데이터 복구 및 최적화**: `grade_records.json` 인코딩 오류(UTF-8 BOM) 수정을 통한 6만 건 데이터 동기화 완료
- **서버 접속성 개선**: 호스트 설정을 `0.0.0.0`으로 변경하여 외부 및 로컬 접속 안정성 확보
- **게스트 채팅 활성화**: `ALLOW_GUEST_CHAT` 설정을 프론트엔드에 완벽 반영하여 로그인 없이도 즉시 채팅 가능
- **긴급 버그 수정**: 시스템 재부팅 후 발생한 `login.js` 구문 오류(Syntax Error) 및 변수 중복 선언 해결
...
## 13. 변경 이력
- **2026-02-10**: Git 병합 완료 (보안 강화 + 게스트 채팅 활성화 + 긴급 버그 수정 통합), 서비스 안정화 및 접근성 개선
